var OrderTypeList = function () {
    this.url = {
        listByPage:"/ordertype/listByPage",
    ...
    this.loadTableData = function () {
        ...
        $('#ordertype').bootstrapTable({
            ...
            columns: [{
                ...
                formatter : function (value, row, index) {
                    var arr = [];
                    arr.push("&nbsp;&nbsp;<a title='编辑' href='/ordertype/edit/"+row.id+"'><i class='fa fa-edit text-primary'></i></a>");
    ...
    this.save = function () {
        ...
        $.ajax({
            type: 'POST',
            url: that.url.create,
            data: $("#ordertype").serialize(),
            success: function (data) {
                if(data.success){
                    Messager.success("操作成功！", function() {
                            location.href='/ordertype/list';
                    });
                }else{
                    Messager.alert("操作失败！");
                }
