var OrderList = function () {
    this.url = {
        listByPage:"/admin/sys/car/petorder/listByPage",
        create:"/admin/sys/car/petorder/create",
        deleteOrder:"/admin/sys/car/petorder/remove",
        updateOrder:"/admin/sys/car/petorder/update",
    ...
    this.loadTableData = function () {
        ...
        $('#petorder').bootstrapTable({
            ...
            columns: [{
                ...
                formatter : function (value, row, index) {
                    var arr = [];
                    arr.push("&nbsp;&nbsp;<a title='编辑' href='/admin/sys/car/petorder/edit/"+row.id+"'><i class='fa fa-edit text-primary'></i></a>");
                    arr.push("&nbsp;&nbsp;<a title='删除' href='javascript:void(0)' onclick='deleteOrder("+row.id+")'><i class='fa fa-trash-o text-warning'></i></a>");
                    return arr;
                }
            }]
        });
                ...
                if(data.length<1){
                    ...
                    for(var i =0;i<data.length;i++){
                        ...
                        var text = "<tr><th>"+data[i].startTime+"</th><th>"+data[i].endTime+"</th><th>"+data[i].maxNum+"</th><th>"+data[i].status+"</th>\n" +
                            "<th>\n" +
                            "<a title='修改' href='javascript:void(0)' onclick=\"petorder.editTimeById("+data[i].id+",'"+data[i].startTime+"','"+data[i].endTime+"',"+data[i].maxNum+")\" ><i class='fa fa-edit text-primary'></i></a>\n" +
