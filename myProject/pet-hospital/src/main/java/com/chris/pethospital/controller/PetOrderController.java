package com.chris.pethospital.controller;

import com.chris.pethospital.entity.OrderType;
import com.chris.pethospital.entity.Pet;
import com.chris.pethospital.entity.PetOrder;
import com.chris.pethospital.entity.User;
import com.chris.pethospital.service.BaseService;
import com.chris.pethospital.service.OrderTypeService;
import com.chris.pethospital.service.PetOrderService;
import com.chris.pethospital.service.PetService;
import com.chris.pethospital.utils.CurrentUtil;
import com.github.pagehelper.PageInfo;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Controller;
import org.springframework.ui.Model;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.PathVariable;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.ResponseBody;

import java.util.HashMap;
import java.util.List;
import java.util.Map;

@Controller
@RequestMapping("/petorder")
public class PetOrderController extends BaseController<PetOrder>{

    @Autowired
    PetOrderService petOrderService;

    @Autowired
    PetService petService;

    @Autowired
    OrderTypeService orderTypeService;

    @Override
    public BaseService<PetOrder> getService() {
        return petOrderService;
    }

    @Override
    public String getViewPrefix() {
        return "petorder";
    }

    @Override
    //进入添加页面
    @GetMapping("/add")
    public String add(String menuId, Model view) {
        Map<String,Object> map = new HashMap<>();
        map.put("status","1");
        map.put("userId", CurrentUtil.getCurrentUser().getId());
        List<OrderType> orderTypes = orderTypeService.getListByParams(map);
        List<Pet> pets = petService.getListByParams(map);
        view.addAttribute("pets",pets);
        view.addAttribute("orderTypes",orderTypes);
        return  getViewPrefix()+"/add";

    }

    //进入编辑页面
    @GetMapping("/edit/{id}")
    public String edit(Model view, @PathVariable Integer id) {
        Map<String,Object> map = new HashMap<>();
        PetOrder obj = getService().getById( id );
        map.put("userId", CurrentUtil.getCurrentUser().getId());
        List<OrderType> orderTypes = orderTypeService.getListByParams(map);
        List<Pet> pets = petService.getListByParams(map);
        view.addAttribute("pets",pets);
        view.addAttribute("orderTypes",orderTypes);
        view.addAttribute(getViewPrefix(),obj);
        return getViewPrefix() + "/edit";
    }

    @RequestMapping("/listMyPage")
    @ResponseBody
    public Object listMyPage( Map<String,Object> params) {
        //根据不同的参数配置,有些传递的是offset
        User user = CurrentUtil.getCurrentUser();
        params.put("userId",user.getId());
        params.put("pageNum", params.get("pageNum"));
        params.put("pageSize",  params.get("pageSize"));
        params.put("keyword",  params.get("keyword"));
        PageInfo info = getService().listByPage(params);

        Map<String,Object> jsonMap = new HashMap<>();
        jsonMap.put("rows", info.getList());
        jsonMap.put("total", info.getTotal());

        return jsonMap;
    }


    @GetMapping(value = "/mylist")
    public String mylist(String menuId) {
        return getViewPrefix() + "/mylist";
    }
}
