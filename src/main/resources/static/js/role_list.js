var RoleList = function () {
    var zNodes ="";
    this.url = {
        listByPage:"/admin/sys/role/listByPage",
        create:"/admin/sys/role/create",
        deleteRole:"/admin/sys/role/remove",
        updateRole:"/admin/sys/role/update",
        addRoleMenus:"/admin/sys/menu/addRoleMenus"
    };
    this.init = function () {
        this.validInit();
    };
    ...
    this.getMenus = function (id) {
        var that = this;
        $.ajax({
            type: 'POST',
            url: '/admin/sys/menu/getAllMenusByRoleId',
            data: {
                id:id
            },
    ...
    this.loadTableData = function () {
        ...
        $('#role').bootstrapTable({
            ...
            columns: [{
                ...
                formatter : function (value, row, index) {
                    var arr = [];
                    arr.push("<a title='权限设置' href='javascript:void(0)' onclick='showArea("+row.id+")'><i class='fa fa-group text-primary'></i></a>");
