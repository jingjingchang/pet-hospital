var StudentList = function () {
    this.url = {
        ...
        addPassList:"/admin/sys/car/pass/create",
    ...
    this.loadTableData = function () {
        ...
        $('#student').bootstrapTable({
            ...
            columns: [{
                ...
                formatter : function (value, row, index) {
                    var arr = [];
                    arr.push("&nbsp;&nbsp;<a title='编辑' href='/admin/sys/car/student/edit/"+row.id+"'><i class='fa fa-edit text-primary'></i></a>");
                    arr.push("&nbsp;&nbsp;<a title='删除' href='javascript:void(0)' onclick='deleteStudent("+row.id+")'><i class='fa fa-trash-o text-warning'></i></a>");
                    return arr;
                }
            }]
        });
            ...
            success: function (data) {
                if(data.status){
                    Messager.success("操作成功！", function() {
                            location.href='/admin/sys/car/student/list';
                    });
                }else{
                    Messager.alert("操作失败！");
                }
